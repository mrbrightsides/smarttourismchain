//console.log('stcdrk_ajax:', stcdrk_ajax);
...
async function makeBooking(bookingId, nama, tanggal, hotel) {
    const signer = await connectWallet();
    const contractAddress = stcdrk_ajax.booking_contract_address;
    const abi = JSON.parse(stcdrk_ajax.booking_contract_abi);
...
document.addEventListener("DOMContentLoaded", function () {
    const form = document.querySelector('#stcdrk-booking-form');
    if (!form) {
        console.warn("Form #stcdrk-booking-form tidak ditemukan");
        return;
    }
    ...
    form.addEventListener('submit', async function (e) {
        ...
        if (metode === 'onchain') {
            ...
            formData.append('action', 'stcdrk_record_onchain');
            formData.append('booking_id', bookingId);
            formData.append('nama', nama);
            formData.append('tanggal', tanggal);
            formData.append('hotel', hotel);
            formData.append('tx_hash', bookingTx);
            formData.append('security', stcdrk_ajax.nonce);

            try {
                const response = await fetch(stcdrk_ajax.ajax_url, {
                    method: 'POST',
                    body: formData
                });
            ...
            formData.append('action', 'stcdrk_offchain');
            formData.append('security', stcdrk_ajax.nonce);
